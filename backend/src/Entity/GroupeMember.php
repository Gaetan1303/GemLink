...
#[ORM\Index(name: 'idx_groupe_member_user_status', fields: ['user', 'status'])]
class GroupeMember
{
    ...
    public function __construct(Groupe $group, User $user, string $role = self::MEMBER)
    {
        ...
        $this->group = $group;
        $this->user = $user;
        $this->joinedAt = $this->createdAt = new DateTimeImmutable();
        $this->changeRole($role);
    }

    public function getId(): Uuid { return $this->id; }
    public function getGroup(): Groupe { return $this->group; }
    ...
    public function isOwner(): bool { return $this->role === self::OWNER; }
    public function isAdmin(): bool { return in_array($this->role, [self::OWNER, self::ADMIN], true); }

    public function changeRole(string $role): self
    {
        if (!in_array($role, [self::OWNER, self::ADMIN, self::MEMBER], true)) throw new InvalidArgumentException('Rôle de faction invalide.');
        $this->role = $role;
        return $this;
    }

    public function leave(string $status = self::LEFT): self
    {
        if (!$this->isActive()) throw new LogicException('Cette adhésion est déjà terminée.');
        if (!in_array($status, [self::LEFT, self::REMOVED], true)) throw new InvalidArgumentException('Statut de départ invalide.');
        $this->status = $status;
