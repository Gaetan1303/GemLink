...
BASE_MODEL_PATH = Path(os.getenv('FINE_TUNE_BASE_MODEL_PATH', os.getenv('VIT_MODEL_PATH', 'checkpoints/vit_stones.pth')))
MODEL_VERSION = os.getenv('FINE_TUNE_MODEL_VERSION', 'unversioned')
...
def main():
    ...
    if BASE_MODEL_PATH.is_file():
        ...
        if isinstance(loaded_checkpoint, dict):
            ...
            if isinstance(loaded_classes, list) and isinstance(loaded_weights, dict):
                base_checkpoint = loaded_checkpoint
                base_classes = [str(label) for label in loaded_classes]
                base_weights = loaded_weights

    # On conserve toutes les sorties du modèle actif, même si le cycle
    # communautaire ne contient que quelques minéraux. Sans ce remapping, un
    # cycle partiel supprimerait silencieusement les classes absentes.
    class_names = list(base_classes)
    class_indexes = {_class_key(label): index for index, label in enumerate(class_names)}
    for folder_label in image_datasets['train'].classes:
        key = _class_key(folder_label)
        if key not in class_indexes:
            class_indexes[key] = len(class_names)
            class_names.append(folder_label)

    for phase, dataset in image_datasets.items():
        unknown = [label for label in dataset.classes if _class_key(label) not in class_indexes]
        if unknown:
            raise ValueError(f"Classes {phase} inconnues : {', '.join(unknown)}")
        target_mapping = {
            source_index: class_indexes[_class_key(label)]
            for label, source_index in dataset.class_to_idx.items()
        }
        dataset.target_transform = target_mapping.__getitem__
    ...
    if base_checkpoint is not None and base_weights is not None:
        if base_classes == class_names:
            ...
            compatible_weights = {
                ...
                and current_weights[key].shape == value.shape
                and not key.startswith('heads.head.')
            }
            model.load_state_dict(compatible_weights, strict=False)
            with torch.no_grad():
                old_head_weight = base_weights.get('heads.head.weight')
                old_head_bias = base_weights.get('heads.head.bias')
                if old_head_weight is not None and old_head_bias is not None:
                    model.heads.head.weight[:len(base_classes)].copy_(old_head_weight)
                    model.heads.head.bias[:len(base_classes)].copy_(old_head_bias)
        print(f" Fine-tuning depuis le checkpoint actif : {BASE_MODEL_PATH}", flush=True)
    ...
    for epoch in range(EPOCHS):
        ...
        for phase in ['train', 'valid']:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                if phase == 'valid':
                    phase_predictions.append(preds.detach().cpu())
                    phase_targets.append(labels.detach().cpu())

            epoch_loss = running_loss / len(image_datasets[phase])
            epoch_acc = running_corrects.double() / len(image_datasets[phase])
            epoch_f1 = 0.0
            if phase == 'valid':
                predictions = torch.cat(phase_predictions)
                targets = torch.cat(phase_targets)
                class_f1_scores = []
                for class_index in targets.unique().tolist():
                    ...
                    class_f1_scores.append((2 * true_positive / denominator) if denominator else 0.0)
                epoch_f1 = sum(class_f1_scores) / len(class_f1_scores)

            print(
                f" └─ {phase.capitalize()} Loss: {epoch_loss:.4f} "
                f"Acc: {epoch_acc:.4f}" + (f" F1: {epoch_f1:.4f}" if phase == 'valid' else ''),
                flush=True,
            )
